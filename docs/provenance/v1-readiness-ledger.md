...
Phase 4PA adds no release-state claim; it records the current user-authorized
scope decision that deeper nested, multi-hop, and broader Context/Resource/why
traversal are deferred post-V1. The bounded deterministic local workflows
already evidenced through Phase 4OY are now the V1 release-maturity boundary,
while candidate validation from an exact commit remains required before any
release flag can change.
...
1. Prepare and run candidate validation from the exact local commit after the
   V1 release gate matrix records bounded Context/Resource/why scope as pass.
2. Refresh the V1 release gate matrix after candidate validation and before any
