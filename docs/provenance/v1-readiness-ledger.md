...
1. Close remaining Resource adapter policies or background re-observation
   ...
   need; after Phase 4MW, prefer case-folding, submodule or sparse-checkout, or
   background scheduling blockers over additional refresh ergonomics.
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  Phase 4MV and Phase 4MW narrow Git worktree Resource adapter policy gaps.
  Phase 4MV exposes no-renames/delete-add behavior for Git renames, while Phase
  4MW exposes tracked symlink handling through Git index/diff material and
  untracked symlink failure as unsupported non-regular Resource error behavior.
  Both runs dogfood against `/tmp` clones of pre-existing `agent_soul` and leave
