...
The source-state basis at the start of Phase 4OY was main commit
`a341b8ca3493af68c767c2ca69dc2ec49553a9e0` and the V1 readiness ledger last
refreshed by Phase 4OW structural-reference nested Plan target Context
Resource recovery. Historical
...
focused blocked-dependency prerequisite Verification Requirements, and Task
scheduling relation edge plus direct create endpoint evolution projection for
`depends_on` and `ordered_before`, direct primary containment create endpoint
evolution projection for queried Goal, Plan, and Task endpoints, plus
normal/full ContextPacket Resource basis recovery hints for runnable same-Plan
peer Tasks and same-Goal cross-Plan peer Tasks when focus is a Task, plus direct
`verifies` relation create endpoint evolution projection for queried
Verification, Acceptance Criterion, and Verification Requirement endpoints,
...
lookup, plus current Plan `why` closure chains for directly contained Tasks
that expose the same AC, VR, Verification, Evidence, and Resource basis fields
and support basis-aware cache refresh from the Plan-discovered Verification id,
plus Phase 4OM read-only proof that a Goal-start recovery can use Goal `why`
to find the direct Plan and then use Plan `why` for the existing basis-aware
recovery path without requiring Goal ancestor rollup, plus Phase 4OP
read-only proof that Plan-focused ContextPacket output already surfaces a
directly contained runnable Task's Resource-backed Verification Requirement,
Resource basis, and basis-aware refresh hint, and that focused
`claim next --context-profile full` preserves that recovery path, plus
Phase 4OQ read-only proof that Goal-focused ContextPacket output already
surfaces the same recovery path for a direct Goal-to-Plan-to-Task shape, plus
Phase 4OR read-only proof that Goal-focused ContextPacket output already
surfaces two direct Goal child Plans' runnable Task Resource-backed recovery
paths and that focused `claim next --context-profile full` preserves the
selected target recovery path, plus Phase 4OS read-only proof that parent
Plan-focused and Goal-focused ContextPacket output already surfaces a
one-level nested SubPlan Task's Resource-backed recovery path and that focused
`claim next --context-profile full` preserves the selected nested Task path,
plus Phase 4OT read-only proof that a focused blocked Task can recover a
different-Goal prerequisite Task's Resource-backed Verification path through
the existing `blocked_dependency` item and that an unfocused `claim next`
selects that runnable prerequisite with the same recovery path, plus
Phase 4OU proof that normal/full ContextPacket output now recovers a direct
Plan/Goal structural reference to a Resource-backed Task without schema,
public CLI flag, `why`, runnable, or `claim next` semantic changes, plus
Phase 4OV proof that normal/full ContextPacket output now recovers a direct
Plan/Goal structural reference to a Plan target's direct Resource-backed Task
without schema, public CLI flag, `why`, runnable, or `claim next` semantic
changes, plus Phase 4OW proof that normal/full ContextPacket output now
recovers a direct Plan/Goal structural reference to a Plan target's one-level
nested Plan child Resource-backed Task without schema, public CLI flag, `why`,
runnable, or `claim next` semantic changes, plus Phase 4OY proof that
normal/full ContextPacket output now recovers a direct Plan/Goal structural
reference to a Plan target's two-level nested Plan child Resource-backed Task
without schema, public CLI flag, `why`, runnable, or `claim next` semantic
changes.
It does not close the
remaining full relation-subject traversal beyond direct endpoint slices,
multi-hop/full evolution traversal, broader context/Resource resolver outside
the bounded direct reference Task and direct referenced-Plan child Task shapes,
the one-level and two-level referenced-Plan nested child Task shapes, broader
