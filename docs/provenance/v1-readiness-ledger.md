...
Last refreshed: 2026-09-07 by Phase 4OY structural-reference two-level nested Plan target Context Resource recovery
...
Phase 4OY adds no release-state claim; it records a pre-change direct
structural-reference two-level nested Plan target Context/Resource recovery gap
and the bounded normal/full ContextPacket fix for direct Plan/Goal references
to Plans whose child Plans contain child Plans that directly contain
Resource-backed Tasks while the release gate matrix remains false.
