...
#[test]
fn context_packet_summarizes_same_goal_cross_plan_peer_resource_recovery_when_focused_on_task() {
    let (_tempdir, path) = store_path();
    let (mut engine, workspace) = create_workspace(&path);
    let branch_id = workspace.initial_branch_id;
    let mut head = workspace.genesis_commit_id;
    ...
    let goal = engine
        .create_goal(
            GoalCreateOptions::new(branch_id, head, "Coordinate cross-plan resource recovery")
                .expect("goal options"),
        )
        .expect("create goal");
    head = goal.commit_id;
    let focused_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
            ...
            .expect("focused plan options"),
        )
        .expect("create focused plan");
    head = focused_plan.commit_id;
    let peer_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
            ...
            .expect("peer plan options"),
        )
        .expect("create peer plan");
    head = peer_plan.commit_id;
    let focused = engine
        .create_task(
    ...
    let containment = engine
        .create_primary_containment(
            PrimaryContainmentCreateOptions::new(
                ...
                head,
                goal.goal_entity_id,
                focused_plan.plan_entity_id,
            )
            .expect("goal-focused-plan containment options"),
        )
        .expect("create goal-focused-plan containment");
    head = containment.commit_id;
    let containment = engine
        .create_primary_containment(
            PrimaryContainmentCreateOptions::new(
                branch_id,
                head,
                goal.goal_entity_id,
                peer_plan.plan_entity_id,
            )
            .expect("goal-peer-plan containment options"),
        )
    ...
    let containment = engine
        .create_primary_containment(
            ...
            .expect("focused-plan-task containment options"),
        )
    ...
    let containment = engine
        .create_primary_containment(
            ...
            .expect("peer-plan-task containment options"),
        )
    ...
    let (basis, baseline_observation_id) = resource_basis(&mut engine, "docs/cross-plan-peer.md");
    let verification = engine
        .create_verification(
            VerificationCreateOptions::new(
                branch_id,
    ...
    assert!(
        !brief
            .items
            .iter()
            .any(|item| item.summary.contains("same_goal_peer_task="))
    );
    ...
    for profile in [ContextProfile::Normal, ContextProfile::Full] {
        let packet = engine
            .context_packet(ContextPacketOptions::new(session.session_id).with_profile(profile))
            .expect("context packet");
        assert_eq!(packet.omission_summary.total, 0);
        let peer_item = packet
            .items
            .iter()
            .find(|item| {
                item.category == ContextItemCategory::VerificationRequirement
                    && item.summary.contains("local_key=VR-cross-plan-resource")
            })
            .unwrap_or_else(|| {
                panic!("missing same-goal peer VR item in {profile:?}: {packet:#?}")
            });
        assert_eq!(peer_item.priority, ContextPriority::P2);
        assert_eq!(
            peer_item.subject.as_ref_string(),
        ...
        assert!(
            peer_item
                .summary
                .contains(&format!("same_goal_peer_task={}", peer.task_entity_id))
        );
        assert!(
            peer_item
                .summary
                .contains(&format!("parent_goal={}", goal.goal_entity_id))
        );
        assert!(
            peer_item
                .summary
                .contains(&format!("focused_plan={}", focused_plan.plan_entity_id))
        );
        assert!(
            peer_item
                .summary
                .contains(&format!("peer_plan={}", peer_plan.plan_entity_id))
        );
        assert!(peer_item.summary.contains("peer_runnable=true"));
        assert!(peer_item.summary.contains(&format!(
            "criterion={}",
...
#[test]
fn context_packet_summarizes_structural_reference_task_resource_recovery_when_focused_on_goal_or_plan()
 {
    let (_tempdir, path) = store_path();
    let (mut engine, workspace) = create_workspace(&path);
    let branch_id = workspace.initial_branch_id;
    ...
    let referrer_goal = engine
        .create_goal(
            GoalCreateOptions::new(branch_id, head, "Goal references reusable work")
    ...
    let referrer_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Plan references reusable work",
                "Recover Resource-backed proof through structural references",
            )
    ...
    let owner_goal = engine
        .create_goal(
            GoalCreateOptions::new(branch_id, head, "Goal owns reusable work")
    ...
    let owner_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Plan owns reusable task",
                "Keep the target outside the referrer containment path",
            )
            .expect("owner plan options"),
        )
        .expect("create owner plan");
    head = owner_plan.commit_id;
    let relation = engine
        .create_primary_containment(
            PrimaryContainmentCreateOptions::new(
                branch_id,
                ...
                owner_plan.plan_entity_id,
            )
            .expect("owner goal-plan containment options"),
        )
        .expect("contain owner plan");
    head = relation.commit_id;
    let referenced_task = engine
        .create_task(
            TaskCreateOptions::new(branch_id, head, "Referenced Resource-backed task")
    ...
    let relation = engine
        .create_primary_containment(
            ...
            .expect("owner plan-task containment options"),
        )
    ...
    let criterion = engine
        .create_acceptance_criterion(
            AcceptanceCriterionCreateOptions::new(
                ...
                "AC-structural-reference-resource",
                "Referenced task proof remains recoverable from focused context.",
                AcceptanceCriterionClassification::Required,
            )
            .expect("acceptance criterion options"),
        )
    ...
    let requirement = engine
        .create_verification_requirement(
            VerificationRequirementCreateOptions::new(
                ...
                criterion.acceptance_criterion_entity_id,
                criterion.acceptance_criterion_entity_version_id,
                "VR-structural-reference-resource",
                "Refresh the structurally referenced Resource basis.",
            )
            .expect("verification requirement options"),
        )
    ...
    let (basis, baseline_observation_id) =
        resource_basis(&mut engine, "docs/structural-reference.md");
    let verification = engine
        .create_verification(
            VerificationCreateOptions::new(
                branch_id,
        ...
        .expect("create structural reference resource-backed verification");
    head = verification.commit_id;
    ...
    let plan_reference = engine
        ...
        .expect("create plan-task structural reference");
    ...
    let goal_reference = engine
        ...
        .expect("create goal-task structural reference");
    ...
    for (focus_entity_id, referrer_kind, relation_id) in [
        ...
        engine
            ...
            .expect("focus session on structural reference referrer");
        ...
        assert!(
            !brief
                .items
                .iter()
                .any(|item| item.summary.contains("structural_reference_task="))
        );
            ...
            let referenced_item = packet
                ...
                .find(|item| {
                    item.category == ContextItemCategory::VerificationRequirement
                        && item
                            .summary
                            .contains("local_key=VR-structural-reference-resource")
                })
                .unwrap_or_else(|| {
                    panic!("missing structural reference VR item in {profile:?}: {packet:#?}")
                });
...
#[test]
fn context_packet_summarizes_structural_reference_plan_target_task_resource_recovery_when_focused_on_goal_or_plan()
 {
    let (_tempdir, path) = store_path();
    let (mut engine, workspace) = create_workspace(&path);
    let branch_id = workspace.initial_branch_id;
    ...
    let referrer_goal = engine
        .create_goal(
            GoalCreateOptions::new(branch_id, head, "Goal references reusable Plan")
                .expect("referrer goal options"),
        )
        .expect("create referrer goal");
    head = referrer_goal.commit_id;
    let referrer_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Plan references reusable Plan",
                "Recover Resource-backed proof through a direct Plan reference",
            )
            .expect("referrer plan options"),
        )
        .expect("create referrer plan");
    ...
    let owner_goal = engine
        .create_goal(
            GoalCreateOptions::new(branch_id, head, "Goal owns referenced Plan")
                .expect("owner goal options"),
        )
        .expect("create owner goal");
    head = owner_goal.commit_id;
    let referenced_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Referenced Plan owns reusable task",
                "Keep the target Task outside the referrer containment path",
            )
    ...
    let referenced_task = engine
        .create_task(
            TaskCreateOptions::new(branch_id, head, "Task under referenced Plan")
                .expect("referenced task options"),
        )
        .expect("create referenced task");
    head = referenced_task.commit_id;
    let plan_task_relation = engine
        .create_primary_containment(
            ...
            .expect("referenced plan-task containment options"),
        )
        .expect("contain referenced task");
    ...
    let criterion = engine
        .create_acceptance_criterion(
            AcceptanceCriterionCreateOptions::new(
                ...
                referenced_task.task_entity_id,
                referenced_task.task_entity_version_id,
                "AC-structural-reference-plan-target-resource",
                "Referenced Plan target task proof remains recoverable from focused context.",
                AcceptanceCriterionClassification::Required,
            )
            .expect("acceptance criterion options"),
        )
    ...
    let requirement = engine
        .create_verification_requirement(
            VerificationRequirementCreateOptions::new(
                ...
                criterion.acceptance_criterion_entity_id,
                criterion.acceptance_criterion_entity_version_id,
                "VR-structural-reference-plan-target-resource",
                "Refresh the referenced Plan target Resource basis.",
            )
            .expect("verification requirement options"),
        )
        .expect("create referenced task verification requirement");
    ...
    let (basis, baseline_observation_id) =
        resource_basis(&mut engine, "docs/structural-reference-plan-target.md");
    let verification = engine
        .create_verification(
            VerificationCreateOptions::new(
                branch_id,
        ...
        .expect("create structural reference Plan target resource-backed verification");
    head = verification.commit_id;
    ...
    for (focus_entity_id, referrer_kind, relation_id) in [
        ...
        engine
            ...
            .expect("focus session on structural reference Plan referrer");
        ...
        assert!(
            !brief
                .items
                .iter()
                .any(|item| item.summary.contains("structural_reference_plan_task="))
        );
        ...
        for profile in [ContextProfile::Normal, ContextProfile::Full] {
            ...
            let referenced_item = packet
                ...
                .find(|item| {
                    item.category == ContextItemCategory::VerificationRequirement
                        && item
                            .summary
                            .contains("local_key=VR-structural-reference-plan-target-resource")
                })
                .unwrap_or_else(|| {
                    panic!(
                        "missing structural reference Plan target VR item in {profile:?}: {packet:#?}"
                    )
                });
            assert_eq!(referenced_item.priority, ContextPriority::P2);
            assert_eq!(
            ...
            assert!(referenced_item.summary.contains(&format!(
                "structural_reference_plan_task={}",
                referenced_task.task_entity_id
            )));
            assert!(
                referenced_item
            ...
            assert!(
                referenced_item
                    .summary
                    .contains("referenced_plan_direct_child=true")
            );
            assert!(referenced_item.summary.contains(&format!(
                "criterion={}",
                criterion.acceptance_criterion_entity_id
...
#[test]
fn context_packet_summarizes_structural_reference_two_level_nested_plan_target_task_resource_recovery_when_focused_on_goal_or_plan()
 {
    let (_tempdir, path) = store_path();
    let (mut engine, workspace) = create_workspace(&path);
    let branch_id = workspace.initial_branch_id;
    ...
    let referrer_goal = engine
        .create_goal(
            GoalCreateOptions::new(
                branch_id,
                head,
                "Goal references reusable two-level nested Plan",
            )
            .expect("referrer goal options"),
        )
        .expect("create referrer goal");
    ...
    let referrer_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Plan references reusable two-level nested Plan",
                "Recover Resource-backed proof through a two-level nested Plan reference",
            )
            .expect("referrer plan options"),
        )
        .expect("create referrer plan");
    ...
    let owner_goal = engine
        .create_goal(
            GoalCreateOptions::new(
                branch_id,
                head,
                "Goal owns referenced two-level nested Plan",
            )
            .expect("owner goal options"),
        )
        .expect("create owner goal");
    ...
    let referenced_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Referenced Plan owns first nested Plan",
                "Keep the target Task two containment hops below the referrer path",
            )
            .expect("referenced plan options"),
        )
        .expect("create referenced plan");
    ...
    let relation = engine
        ...
        .expect("contain referenced plan");
    head = relation.commit_id;
    let first_nested_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "First nested Plan owns second nested Plan",
                "First containment hop below the structural reference target",
            )
            .expect("first nested plan options"),
        )
        .expect("create first nested plan");
    head = first_nested_plan.commit_id;
    let referenced_plan_relation = engine
        .create_primary_containment(
            PrimaryContainmentCreateOptions::new(
                branch_id,
                head,
                referenced_plan.plan_entity_id,
                first_nested_plan.plan_entity_id,
            )
            .expect("referenced plan-first nested plan containment options"),
        )
        .expect("contain first nested plan");
    head = referenced_plan_relation.commit_id;
    let second_nested_plan = engine
        .create_plan(
            PlanCreateOptions::new(
                branch_id,
                head,
                "Second nested Plan owns reusable task",
                "Second containment hop below the structural reference target",
            )
            .expect("second nested plan options"),
        )
        .expect("create second nested plan");
    head = second_nested_plan.commit_id;
    let nested_plan_relation = engine
        .create_primary_containment(
            PrimaryContainmentCreateOptions::new(
                branch_id,
                head,
                first_nested_plan.plan_entity_id,
                second_nested_plan.plan_entity_id,
            )
            .expect("first nested-second nested plan containment options"),
        )
        .expect("contain second nested plan");
    head = nested_plan_relation.commit_id;
    let referenced_task = engine
        .create_task(
            TaskCreateOptions::new(
                branch_id,
                head,
                "Task under second-level nested referenced Plan",
            )
            .expect("referenced task options"),
        )
        .expect("create referenced task");
    ...
    let plan_task_relation = engine
        .create_primary_containment(
            ...
            .expect("second nested plan-task containment options"),
        )
        .expect("contain referenced task");
    head = plan_task_relation.commit_id;
    ...
    let criterion = engine
        .create_acceptance_criterion(
            AcceptanceCriterionCreateOptions::new(
                ...
                referenced_task.task_entity_id,
                referenced_task.task_entity_version_id,
                "AC-structural-reference-two-level-nested-plan-resource",
                "Two-level nested referenced Plan target task proof remains recoverable from focused context.",
                AcceptanceCriterionClassification::Required,
            )
            .expect("acceptance criterion options"),
        )
    ...
    let requirement = engine
        .create_verification_requirement(
            VerificationRequirementCreateOptions::new(
                ...
                criterion.acceptance_criterion_entity_id,
                criterion.acceptance_criterion_entity_version_id,
                "VR-structural-reference-two-level-nested-plan-resource",
                "Refresh the two-level nested referenced Plan target Resource basis.",
            )
            .expect("verification requirement options"),
        )
        .expect("create referenced task verification requirement");
    ...
    let (basis, baseline_observation_id) = resource_basis(
        &mut engine,
        "docs/structural-reference-two-level-nested-plan-target.md",
    );
    let verification = engine
        .create_verification(
            VerificationCreateOptions::new(
        ...
        .expect(
            "create structural reference two-level nested Plan target resource-backed verification",
        );
    head = verification.commit_id;
    ...
    for (focus_entity_id, referrer_kind, relation_id) in [
        ...
        engine
            ...
            .expect("focus session on structural reference two-level nested Plan referrer");
        ...
        assert!(!brief.items.iter().any(|item| {
            item.summary
                .contains("structural_reference_two_level_nested_plan_task=")
        }));
        ...
        for profile in [ContextProfile::Normal, ContextProfile::Full] {
                ...
                .find(|item| {
                    item.category == ContextItemCategory::VerificationRequirement
                        && item.summary.contains(
                            "local_key=VR-structural-reference-two-level-nested-plan-resource",
                        )
                })
                .unwrap_or_else(|| {
                    panic!(
                        "missing structural reference two-level nested Plan target VR item in {profile:?}: {packet:#?}"
                    )
                });
            assert_eq!(referenced_item.priority, ContextPriority::P2);
            assert_eq!(
            ...
            assert!(referenced_item.summary.contains(&format!(
                ...
                referenced_plan_relation.relation_id
            )));
            assert!(referenced_item.summary.contains(&format!(
                "nested_plan_containment_relation_id={}",
                nested_plan_relation.relation_id
            )));
            assert!(referenced_item.summary.contains(&format!(
                "plan_task_containment_relation_id={}",
                plan_task_relation.relation_id
            ...
            assert!(
                referenced_item
                    .summary
                    .contains("referenced_plan_two_level_child=true")
            );
            assert!(referenced_item.summary.contains(&format!(
                "criterion={}",
                criterion.acceptance_criterion_entity_id
