...
Last refreshed: 2026-09-01 by ADR-0426 / Phase 4LK
...
| Context resolver | Yes | Partial | Yes | Partial | Phase 4LK adds brief blocked dependency packet items with blocking Task status/priority/description; complete remaining V1 categories: Goal/Plan path packets, Attempt details, path-sensitive Knowledge policy, and packet persistence. |
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  Task id as the next claim target. WorkVCS has still not been used for another
  real project.
