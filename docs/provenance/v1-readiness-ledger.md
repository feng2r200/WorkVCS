...
1. Close remaining Resource adapter policies or background re-observation
   ...
   need; after Phase 4MX, prefer case-folding, sparse-checkout, or background
   scheduling blockers over additional refresh ergonomics.
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  Phase 4MV, Phase 4MW, and Phase 4MX narrow Git worktree Resource adapter
  policy gaps. Phase 4MV exposes no-renames/delete-add behavior for Git
  ...
  error behavior, and Phase 4MX exposes parent-Git submodule gitlink/status/diff
  observation with recursion disabled. All three runs dogfood against `/tmp`
  clones of pre-existing `agent_soul` and leave the original repository
  unchanged; Resource remains blocking because case-folding, sparse-checkout
