...
Last refreshed: 2026-09-02 by ADR-0466 / Phase 4MY
...
1. Close remaining Resource adapter policies or background re-observation
   ...
   need; after Phase 4MY, prefer case-folding or background scheduling blockers
   over additional refresh ergonomics.
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  Phase 4MV, Phase 4MW, Phase 4MX, and Phase 4MY narrow Git worktree Resource
  adapter policy gaps. Phase 4MV exposes no-renames/delete-add behavior for Git
  renames, Phase 4MW exposes tracked symlink handling through Git index/diff
  material and untracked symlink failure as unsupported non-regular Resource
  error behavior, Phase 4MX exposes parent-Git submodule gitlink/status/diff
  observation with recursion disabled, and Phase 4MY exposes parent-Git
  sparse-checkout index/status/diff observation with expansion disabled. All
  four runs dogfood against `/tmp` clones of pre-existing `agent_soul` and leave
  the original repository unchanged; Resource remains blocking because
  case-folding semantics and background re-observation scheduling remain
  unproven.
