...
use super::record::{
    ...
    RECORD_FINDING_CORRECTION_OPERATION_SCHEMA_VERSION, RECORD_FINDING_INVALIDATE_OPERATION_TYPE,
    RECORD_FINDING_SUPERSEDE_OPERATION_TYPE, RECORD_RELATION_CREATE_OPERATION_SCHEMA_VERSION,
    RECORD_RELATION_CREATE_OPERATION_TYPE, RECORD_RELATION_REMOVE_OPERATION_SCHEMA_VERSION,
    RECORD_RELATION_REMOVE_OPERATION_TYPE, RECORD_RELATION_RESTORE_OPERATION_SCHEMA_VERSION,
...
fn validate_entity_transition_changeset(
    ...
    match operation_type.as_str() {
        ...
        KNOWLEDGE_RELATION_CREATE_OPERATION_TYPE => {
            if operation_schema_version != KNOWLEDGE_RELATION_CREATE_OPERATION_SCHEMA_VERSION {
                return Err(WorkVcsError::ReplayUnsupported(format!(
                    "normal ChangeSet {changeset_id} operation schema version {operation_schema_version} is deferred"
                )));
            }
        ...
        RECORD_RELATION_RESTORE_OPERATION_TYPE => {
            if operation_schema_version != RECORD_RELATION_RESTORE_OPERATION_SCHEMA_VERSION {
                return Err(WorkVcsError::ReplayUnsupported(format!(
                    "normal ChangeSet {changeset_id} operation schema version {operation_schema_version} is deferred"
                )));
            }
        ...
        RECORD_FINDING_SUPERSEDE_OPERATION_TYPE | RECORD_FINDING_INVALIDATE_OPERATION_TYPE => {
            if operation_schema_version != RECORD_FINDING_CORRECTION_OPERATION_SCHEMA_VERSION {
                return Err(WorkVcsError::ReplayUnsupported(format!(
                    "normal ChangeSet {changeset_id} operation schema version {operation_schema_version} is deferred"
                )));
            }
