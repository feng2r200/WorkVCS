...
Last refreshed: 2026-09-07 by Phase 4OV structural-reference Plan target Context Resource recovery
...
Phase 4OV adds no release-state claim; it records a pre-change direct
structural-reference Plan target Context/Resource recovery gap and the bounded
normal/full ContextPacket fix for direct Plan/Goal references to Plans that
directly contain Resource-backed Tasks while the release gate matrix remains
false.
