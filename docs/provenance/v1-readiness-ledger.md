...
Last refreshed: 2026-09-07 by Phase 4OW structural-reference nested Plan target Context Resource recovery
...
Phase 4OW adds no release-state claim; it records a pre-change direct
structural-reference nested Plan target Context/Resource recovery gap and the
bounded normal/full ContextPacket fix for direct Plan/Goal references to Plans
whose one-level child Plans directly contain Resource-backed Tasks while the
