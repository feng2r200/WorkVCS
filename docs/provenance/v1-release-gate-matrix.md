...
Phase 4PC refreshes this matrix after Phase 4PB candidate validation passed from
exact commit `5f12ebd4386377bfeba5998c146b36fe5d3c4b67`. The validation log at
`/tmp/workvcs-4pb-candidate-validation-prep-rerun-20260907T065403Z` records
`candidate_validation=PASS`, `checks_total=13`, `checks_passed=13`,
`checks_failed=0`, `git_dirty_after=0`, `head_at_end` and `main_ref_at_end`
matching the candidate commit, and `release_flag_positive_hits=0` before this
matrix refresh.

The checks cover current governance status, docs gate consistency, `git diff
--check`, `scripts/validate-schema-v0.1.sh`, `cargo fmt --all -- --check`,
`cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo
test --workspace --all-targets`, `scripts/smoke-v0.1-cli-workflow.sh`, larger
Store portability, maintained Store portability, operator recovery maturity,
the Phase 4OM Goal-start recovery probe, and the Phase 4OX two-level nested Plan
target Context/Resource recovery probe.

The Phase 4OM probe records
`probe_result=RECOVERY_SUPPORTED_WITH_PLAN_HOP`. The Phase 4OX probe records
`probe_result=STRUCTURAL_REFERENCE_TWO_LEVEL_NESTED_PLAN_TARGET_CONTEXT_RESOURCE_RECOVERY_SUPPORTED`.
The Phase 4PC independent review at
`/tmp/workvcs-4pc-candidate-validation-governance-refresh-20260907T065913Z`
records `blocking_findings=0` and supports turning on
`V0_1_DOGFOOD_COMPLETE` and `RELEASE_CANDIDATE_ALLOWED` while keeping
`V1_RELEASE_READY=false`.

The release decision after Phase 4PC is:
...
`RELEASE_CANDIDATE_ALLOWED=true` means the local bounded candidate has passed
the required local validation and governance refresh. It does not authorize
