# Phase 4OW Structural Reference Nested Plan Target Context Resource Probe
...
Phase 4OW checks a bounded structural-reference Context/Resource resolver
boundary left open after Phase 4OV: a focused Plan or Goal directly
`references` another Plan outside its containment path, that referenced Plan
directly contains one nested Plan, and the nested Plan directly contains a Task
with a current-head Resource-backed Verification Requirement.

The pre-change probe found a concrete recovery gap in ContextPacket output.
The implementation closes only this one-reference-hop plus one containment-hop
nested Plan target path for normal/full context packets. It does not recurse
through arbitrary Plan depth, follow multi-hop references, or change Store
...
- Current main at pre-change probe start:
  `33d28ba6a417a44e67ae6851de565aa2af52ac07`
...
- Implementation validation log dir:
  `/tmp/workvcs-4ow-implementation-validation-20260907T053857Z`
...
- one nested Plan directly contained by the referenced Plan;
- one Task directly contained by that nested Plan;
...
One local probe/validation attempt after Phase 4OV reused the Phase 4OV probe
shape instead of the Phase 4OW nested Plan target shape. It produced only
`/tmp` output, did not modify repository state, and is not used as Phase 4OW
behavioral evidence. The effective Phase 4OW evidence is the pre-change and
post-change log dirs listed above.
...
This proves direct Plan/Goal-to-Plan structural references and the
Plan-to-Plan containment path were accepted and visible through existing
...
recover the referenced Plan, nested Plan, nested Task, or refresh hint.

## Implementation

The Phase 4OW implementation:

- reuses Phase 4OU and Phase 4OV structural-reference ContextOverview data;
- for normal/full context packets only, when the Session focus is a Plan or
  ...
  Plans;
- preserves Phase 4OV direct child Task recovery from the referenced Plan;
- additionally inspects one level of child Plans directly contained by that
  referenced Plan;
- reuses the existing Resource-backed Verification Requirement summary
  renderer for Tasks directly contained by those one-level child Plans;
- marks the summary with `structural_reference_nested_plan_task=...`,
  `referrer=...`, `referrer_kind=...`, `referenced_plan=...`,
  `nested_plan=...`, `relation_id=...`,
  `plan_plan_containment_relation_id=...`,
  `plan_task_containment_relation_id=...`, `reference_direct=true`, and
  `referenced_plan_one_level_child=true`;
- preserves brief context behavior and all existing direct Task reference,
  direct Plan target, same-Plan peer, same-Goal peer, and blocked-dependency
  summary markers.
...
Phase 4OW closes the direct Plan/Goal structural-reference-to-Plan-target plus
one-level nested Plan child Task Context/Resource recovery gap for the tested
bounded shape. A continuation operator focused on the referrer Plan or Goal
can see the referenced Plan, nested Plan, nested Task, Resource-backed
