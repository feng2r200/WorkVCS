...
use workvcs_core::{
    ...
    KnowledgeSpaceSourceStaleExposuresOptions, KnowledgeTransitionOptions, StoreInitOptions,
...
fn create_workspace(path: &Path) -> (Engine, WorkspaceInfo) {
    let mut engine = Engine::init(
        path,
        StoreInitOptions::new("phase4ai-source-stale-exposure-store").expect("store options"),
    )
    .expect("init engine");
...
#[test]
fn source_stale_exposures_include_only_active_stale_source_bindings() {
    ...
    let withdrawn_knowledge = create_knowledge(
        ...
        "Withdrawn stale reusable knowledge",
    );
    ...
    engine
        .transition_knowledge(
            KnowledgeTransitionOptions::invalidate(
                workspace.initial_branch_id,
                stale_transition.commit_id,
                withdrawn_knowledge.knowledge_entity_id,
                withdrawn_knowledge.knowledge_entity_version_id,
                "withdrawn source drift",
            )
            .expect("invalidate withdrawn-source options"),
        )
        .expect("invalidate withdrawn source");
    let withdrawn_stale = engine
        .refresh_knowledge_exposure_source_status(KnowledgeExposureRefreshSourceStatusOptions::new(
            withdrawn_exposure.exposure_id,
        ))
        .expect("refresh withdrawn source")
        .exposure;
    assert_eq!(
        withdrawn_stale.source_status.source_status.as_str(),
        "stale"
    );
    engine
        .withdraw_knowledge_exposure(
            KnowledgeExposureWithdrawOptions::new(
                withdrawn_stale.exposure_id,
                withdrawn_stale.transition_id,
            )
            .expect("withdraw options"),
        )
        .expect("withdraw exposure");

    let source_stale = engine
        .knowledge_space_source_stale_exposures(KnowledgeSpaceSourceStaleExposuresOptions::new(
            knowledge_space.knowledge_space_id,
        ))
        .expect("source-stale exposures");

    assert_eq!(
        source_stale.knowledge_space_id,
        knowledge_space.knowledge_space_id
    );
    assert_eq!(source_stale.exposures.len(), 1);
    assert_eq!(
        source_stale.exposures[0].exposure_id,
        stale_exposure.exposure_id
    );
    assert_ne!(
        source_stale.exposures[0].exposure_id,
