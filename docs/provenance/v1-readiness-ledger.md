...
Phase 5B repairs that creation path and adopts the deliberately incompatible
local Bundle v2 profile. Top-level `verify --evidence-content` now preflights
before persisting raw bytes and records a contract-valid local storage
location. Bundle export distinguishes metadata-only Evidence content from
portable local bodies, verifies each advertised object, writes raw bodies under
`objects/<digest>.bin`, and fails closed for missing or tampered objects. A
same-Store copied-target apply persists those bodies into the target Store's
local object directory and records their storage locations transactionally;
digest-only content stays intentionally unextractable. Focused core and CLI
regressions, the complete workspace suite, library/binary Clippy with the
repository's pre-existing argument-count lint excluded, and a globally
installed CLI dogfood all pass. The installed dogfood exported 15 payload
files with 34 references and one raw object, imported one target storage
location, extracted the exact 52-byte verification Evidence body, and left
both source and target Stores valid. Bundle v1 is intentionally rejected
rather than silently importing metadata while discarding raw Evidence.

Phase 5C corrects two maintained-Store recovery failures exposed immediately
after Phase 5B dogfood. Brief Context construction previously called the full
`why` evolution projection once per Knowledge Entity; on the maintained Store,
the installed CLI remained at full CPU for more than two minutes without
returning. The resolver now replays state once and reads only the
`knowledge_exposure_derived_from` edges needed by Context. Brief Context also
stops classifying every historical terminal Task as current, while retaining a
terminal Task when the active Session explicitly focuses or claims it. The same
real `resume --budget-items 80` now completes in 2.22 seconds, reduces available
items from 114 to 34, preserves all four active Task paths and their AC/VR
...
- required TUI, GUI, or human-first storage; and
- deeper nested Plan traversal beyond the currently evidenced two-level Plan
  target boundary, multi-hop structural references, broader
  Context/Resource/why traversal, full relation-subject traversal, multi-hop or
  full evolution traversal, and broader causal traversal unless a later explicit
  scope decision reopens them.

## Evidence Snapshot

- Phase 4PE adds compact read-only `workvcs resume` output over existing brief
  ContextPacket data as the first post-V1 local improvement after the Phase
  4PD release-ready judgment. The implementation changes only
  `crates/workvcs-cli/src/main.rs` and records ADR/provenance evidence in
  `docs/decisions/adr/0496-phase-4pe-compact-read-only-resume-query.md` and
  `docs/provenance/phase-4pe-compact-read-only-resume-query.md`. Validation at
  `/tmp/workvcs-resume-slice-20260907T092600Z` records targeted resume/help
  tests, full CLI and workspace tests with `RUST_MIN_STACK=33554432`, and
  `git diff --check` passing. The public CLI integration test proves
  `resume` surfaces Goal/Plan path, current work, blocked dependency,
  Resource-basis recovery, and next-action signals while leaving Branch head,
  state digest, history, events, context-packet snapshots, and Session claims
  unchanged. Phase 4PE does not change the release gate matrix or authorize
  release, tag, push, deploy, remote, production, credential, or
  `/usr/local/bin/workvcs` installation actions.
...
- Phase 4PB validates candidate commit
  ...
  matrix at
  `/tmp/workvcs-4pb-candidate-validation-prep-rerun-20260907T065403Z`. The
  fresh validation records `candidate_validation=PASS`, `checks_total=13`,
  `checks_passed=13`, `checks_failed=0`, `git_dirty_after=0`,
  `release_flag_positive_hits=0`, Phase 4OM
  `probe_result=RECOVERY_SUPPORTED_WITH_PLAN_HOP`, and Phase 4OX
  `probe_result=STRUCTURAL_REFERENCE_TWO_LEVEL_NESTED_PLAN_TARGET_CONTEXT_RESOURCE_RECOVERY_SUPPORTED`.
  Phase 4PC independently reviews that evidence with `blocking_findings=0` at
  `/tmp/workvcs-4pc-candidate-validation-governance-refresh-20260907T065913Z`.
  This supports `V0_1_DOGFOOD_COMPLETE=true` and
  `RELEASE_CANDIDATE_ALLOWED=true`; it does not authorize changing
  `V1_RELEASE_READY` to true or any release, tag, push, deploy, remote,
  production, or credential operation.
- Phase 4PA records the current user-authorized V1 bounded scope decision:
  deeper nested Plan traversal beyond the currently evidenced two-level Plan
  target boundary, multi-hop structural references, broader
  Context/Resource/why traversal, full relation-subject traversal,
  multi-hop/full evolution traversal, and broader causal traversal are deferred
  post-V1. This is a governance release-scope refresh only; it does not change
  code, Store schema, ContextPacket schema, public CLI flags, ADR semantics,
  tests, runtime behavior, release state, or remote state.
- `workvcs --help` currently exposes the broad V0.1 command surface, including
  ...
  Resource, Record, Session, Claim, Context, Resume, Next, Runnable, the
  high-level `verify` wrapper, Verification, Projection, Bundle, Checkpoint,
  and Merge command families.
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  evolution traversal, broader causal traversal, and broader context/Resource
  resolver maturity remain open, so the release state remains false.
  Phase 4OM is a read-only current-main probe for the Goal-start recovery
  question left by Phase 4OK. The effective probe at
  `/tmp/workvcs-4om-goal-plan-recovery-probe-20260904T095537Z` proves
  `phase4om_probe=PASS`,
  `probe_result=RECOVERY_SUPPORTED_WITH_PLAN_HOP`,
  `concrete_implementation_gap_found=false`, `commands_exit_failures=0`,
  `failed_assertions=0`, `task_why_closure_chains=1`,
  ...
  `ac_status_after_plan_refresh=verified`, and
  `release_flag_positive_hits=0`. The earlier probe attempt at
  `/tmp/workvcs-4om-goal-plan-recovery-probe-20260904T095105Z` is retained
  only as a stale-binary failed attempt and is not used as behavioral evidence.
  Phase 4OM does not change code, schema, CLI flags, tests, ADR authority, or
  release state; it supports keeping Goal ancestor rollup and broader traversal
  out of scope until a later real workflow shows a concrete blockage.
  Phase 4OP is a read-only current-main probe for Plan-start Context/Resource
  recovery. The effective probe at
  `/tmp/workvcs-4op-plan-start-context-resource-probe-20260907T035327Z`
  proves `probe_execution_status=PASS`,
  `probe_result=PLAN_START_DIRECT_CONTEXT_RESOURCE_RECOVERY_SUPPORTED`,
  `gap_kind=none`, `focus_plan_supported=true`,
  ...
  `doctor_required_valid_exit=0`, `git_dirty_lines_after=0`, and
  `release_flag_positive_hits=0`. Phase 4OP does not change code, schema,
  CLI flags, tests, ADR authority, or release state; it supports keeping
  broader Context/Resource resolver work out of scope until a later real
  workflow shows a concrete blockage.
  Phase 4OQ is a read-only current-main probe for Goal-start Context/Resource
  recovery. The effective probe at
  `/tmp/workvcs-4oq-goal-start-context-resource-probe-20260907T040620Z`
  proves `probe_execution_status=PASS`,
  `probe_result=GOAL_START_DIRECT_CONTEXT_RESOURCE_RECOVERY_SUPPORTED`,
  `gap_kind=none`, `focus_goal_supported=true`,
  ...
  `release_flag_positive_hits=0`. Phase 4OQ does not change code, schema,
  CLI flags, tests, ADR authority, or release state; it supports keeping
  broader Context/Resource resolver work out of scope until a later real
  workflow shows a concrete blockage.
  Phase 4OR is a read-only current-main probe for Goal-start multi-Plan
  Context/Resource recovery. The effective probe at
  ...
  `focus_goal_supported=true`, `goal_focus_full_has_target_vr_key=true`,
  ...
  `doctor_required_valid_exit=0`, `repo_git_dirty_lines_after=0`, and
  `release_flag_positive_hits=0`. Phase 4OR does not change code, schema,
  CLI flags, tests, ADR authority, or release state; it supports keeping
  broader Context/Resource resolver work out of scope until a later real
  workflow shows a concrete blockage.
  Phase 4OS is a read-only current-main probe for one-level nested Plan
  Context/Resource recovery. The effective probe at
  ...
  `focus_goal_supported=true`, `goal_focus_full_has_vr_key=true`,
  `goal_focus_full_has_resource_basis=true`,
  `goal_focus_full_has_refresh_hint=true`,
  ...
  `cache_refresh_applicability=applicable`,
  `doctor_required_valid_exit=0`, `repo_git_dirty_lines_after=0`, and
  `release_flag_positive_hits=0`. Phase 4OS does not change code, schema,
  CLI flags, tests, ADR authority, or release state; it supports keeping
  broader Context/Resource resolver work out of scope until a later real
  workflow shows a concrete blockage.
  Phase 4OT is a read-only current-main probe for cross-Goal Task-dependency
  Context/Resource recovery. The effective probe at
  ...
  `claim_next_task_matches=true`, `claim_next_has_vr_key=true`,
  `claim_next_has_resource_basis=true`,
  `claim_next_has_refresh_hint=true`,
  `cache_refresh_applicability=applicable`,
  `doctor_required_valid_exit=0`, `repo_git_dirty_lines_after=0`, and
  `release_flag_positive_hits=0`. Phase 4OT does not change code, schema,
  CLI flags, tests, ADR authority, or release state; it supports keeping
  broader Context/Resource resolver work out of scope until a later real
  workflow shows a concrete blockage.
  Phase 4OU narrows a direct structural-reference Context/Resource recovery
  gap. The pre-change public CLI probe at
  ...
  `plan_context_has_vr=false`, `plan_context_has_resource_basis=false`,
  `plan_context_has_refresh_hint=false`, `goal_context_has_vr=false`,
  `goal_context_has_resource_basis=false`,
  `goal_context_has_refresh_hint=false`,
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, `repo_dirty=0`, and
  `release_positive_hits=0`. The Phase 4OU binary keeps brief context,
  schema, public CLI flags, `why`, runnable, `claim next`, and structural
  reference semantics unchanged while adding normal/full ContextPacket
  Resource-backed Verification Requirement recovery hints for direct
  Plan/Goal structural references to Tasks. The post-change public CLI probe
  at
  ...
  Phase 4OU does not prove structural-reference Plan targets, multi-hop
  ...
  causal traversal, or release state.
  Phase 4OV narrows the direct Plan-target version of that
  structural-reference Context/Resource recovery gap. The pre-change public
  CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, `repo_dirty=0`, and
  `release_positive_hits=0`. The Phase 4OV binary keeps brief context,
  schema, public CLI flags, `why`, runnable, `claim next`, and structural
  reference semantics unchanged while adding normal/full ContextPacket
  Resource-backed Verification Requirement recovery hints for direct
  Plan/Goal structural references to Plans with direct child Tasks. The
  post-change public CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, and `release_positive_hits=0`.
  Phase 4OV does not prove nested referenced Plan traversal, multi-hop
  structural references, broad Resource resolver maturity, full
  relation-subject traversal, multi-hop/full evolution traversal, broader
  causal traversal, or release state.
  Phase 4OW narrows the one-level nested Plan target version of that
  structural-reference Context/Resource recovery gap. The pre-change public
  CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, `repo_dirty=0`, and
  `release_positive_hits=0`. A local drift attempt reused the Phase 4OV probe
  shape, produced only `/tmp` output, did not modify repository state, and is
  not used as 4OW evidence. The Phase 4OW binary keeps brief context, schema,
  public CLI flags, `why`, runnable, `claim next`, and structural reference
  semantics unchanged while adding normal/full ContextPacket Resource-backed
  Verification Requirement recovery hints for direct Plan/Goal structural
  references to Plans whose one-level child Plans directly contain Tasks. The
  post-change public CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, and `release_positive_hits=0`.
  Phase 4OW does not prove deeper nested Plan traversal, multi-hop structural
  references, broad Resource resolver maturity, full relation-subject
  traversal, multi-hop/full evolution traversal, broader causal traversal, or
  release state.
  Phase 4OY narrows the two-level nested Plan target version of that
  structural-reference Context/Resource recovery gap. The pre-change public
  CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, `repo_dirty=0`, and
  `release_positive_hits=0`. The Phase 4OY binary keeps brief context,
  schema, public CLI flags, `why`, runnable, `claim next`, and structural
  reference semantics unchanged while adding normal/full ContextPacket
  Resource-backed Verification Requirement recovery hints for direct
  Plan/Goal structural references to Plans whose child Plans contain child
  Plans that directly contain Tasks. The post-change public CLI probe at
  ...
  `cache_refresh_applicability=applicable`,
  `cache_refresh_resource_stamps=1`, and `release_positive_hits=0`.
  Phase 4OY does not prove deeper nested Plan traversal, multi-hop structural
  references, broad Resource resolver maturity, full relation-subject
  traversal, multi-hop/full evolution traversal, broader causal traversal, or
  release state.
