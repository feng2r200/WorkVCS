...
pub const OBJECT_STORE_FORMAT_VERSION: i64 = 2;
...
pub(crate) fn load_store_info_for_local_object_v2_migration(
    ...
    validate_version("schema_version", manifest.schema_version, SCHEMA_VERSION)?;
    if manifest.object_store_format_version != 1
        && manifest.object_store_format_version != OBJECT_STORE_FORMAT_VERSION
    {
        return Err(WorkVcsError::StoreCompatibilityUnsupported(format!(
            "object_store_format_version {} is not a supported migration source or target",
            manifest.object_store_format_version
        )));
    }
    validate_exact("id_scheme", &manifest.id_scheme, ID_SCHEME)?;
    validate_exact(
        "digest_algorithm",
        &manifest.digest_algorithm,
        DIGEST_ALGORITHM,
