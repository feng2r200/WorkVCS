...
Last refreshed: 2026-09-01 by ADR-0425 / Phase 4LJ
...
| Context resolver | Yes | Partial | Yes | Partial | Phase 4LJ adds brief current-task AC/VR obligation packets for `claim next` and `context`; complete remaining V1 categories: Goal/Plan path packets, richer blocker context, Attempt details, path-sensitive Knowledge policy, and packet persistence. |
...
1. Complete the remaining V1 Context Resolver categories when the verify,
   handoff, and claim-next surfaces expose real packet content.
...
- Current smoke Stores are temporary and scripted. Phase 4KX adds a local
  ...
  with the `verify` wrapper until `ac status` returns `verified`. WorkVCS has
  still not been used for another real project.
